...
1. 6 components ("factors") explain at least 50% (52.29%) of the variance of the dataset.
2. 5 yr T-Notes US (0.93); 10yr T-Notes (0.93); Treasury Bonds (0.86); 2 yr T-Note US (0.85); Canadian Bond (0.79) 
3. The first 10 components explain 62.61% of the variance and could be called:
**Factor 1** Government bonds
**Factor 2** FX
**Factor 3** Stock indices (EU)
**Factor 4** Stock indices (North-Amercican)
...
4. In the seperate sections below the principal components and screen plot of the the pre-crisis bull market years are plot (November 1, 2002, and October 1, 2007) and the financial crisis years ( October 1, 2007 and  March 1, 2009)
5. In the bull market more components make up the line before the elbow in the scree chart, more components influence the market. It can be seen for example that component 1 is less significant compared with the average over all the years. In the bear market you need only the first 4 components to explain 50% of the variance. Also, the correlation between futures increased.
...
1. The first lines set the signal to zero, and then define a frequency (which will be used later to calculate the moving averages). Next, the moving average (function 'ma') is used, over a defined window with the frequency above. The output of this function is a signal, which is given by the sign of the output (negative, neutral, positive). The weight of each signal is calculated by scaling the sum across all the futures, so that the total sum is 1. Then, the investment strategy is applied: we invest by applying the weights we calculated 2 days ago to the futures.
2. The command line MAfreq<-seq(10,250,by=20), affects how often the trading strategy is implemented, it affects the frequency. If a higher frequency is used, the strategy becomes less sensitive, returns are reduced, and DD is increased. 
